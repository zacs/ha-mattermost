...
class MattermostFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            error, info = await self._async_try_connect(user_input[CONF_API_KEY], user_input[CONF_URL], user_input[CONF_DEFAULT_CHANNEL])
    ...
    async def _async_try_connect(
        ...
        try:
            ...
            if not clean_url.startswith(('http://', 'https://')):
                ...
                if clean_url.startswith('192.168.') or clean_url.startswith('10.') or clean_url.startswith('127.') or 'localhost' in clean_url:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        _LOGGER.debug("Bot token validation successful via client config")
                ...
                async with session.get(
                    ...
                    _LOGGER.debug(f"No bearer prefix response status: {response.status}")
                    
                    if response.status == 200:
                        _LOGGER.debug("Token validation successful without Bearer prefix")
                        server_name = f"Bot@{parsed_url.hostname}"
                        return None, {"server": server_name}
                ...
                async with session.get(
                    ...
                    _LOGGER.debug(f"X-Token header response status: {response.status}")
                ...
                # Method 4: Try a POST request to webhooks endpoint (common bot permission)
                ...
                async with session.get(
                    ...
                    _LOGGER.debug(f"Webhooks endpoint response status: {response.status}")
